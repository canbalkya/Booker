...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    let randomImage = [UIImage(named: "red")!, UIImage(named: "blue")!, UIImage(named: "green")!, UIImage(named: "purple")!, UIImage(named: "orange")!].randomElement()!
    ...
    func addBook(name: String, topic: String, author: String, isRead: Bool, timestamp: Date, image: UIImage) {
    ...
    func fetchBooks() {
        ...
        do {
            ...
            for item in fetchResults as! [NSManagedObject] {
                books.append(Book(booksName: item.value(forKey: "name") as! String, booksTopic: item.value(forKey: "topic") as! String, booksAuthor: item.value(forKey: "author") as! String, isRead: item.value(forKey: "isRead") as? Bool ?? true, timestamp: item.value(forKey: "timestamp") as? Date ?? Date(), image: item.value(forKey: "image") as? UIImage ?? UIImage(named: "red")!))
            }
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let delete = UIContextualAction(style: .destructive, title: "Delete") { (action, UIView, (Bool) -> Void) in
            self.removeBook(name: self.books[indexPath.row].booksName, topic: self.books[indexPath.row].booksTopic, author: self.books[indexPath.row].booksAuthor, isRead: self.books[indexPath.row].isRead, timestamp: self.books[indexPath.row].timestamp)
    ...
    @IBAction func addButton(_ sender: UIBarButtonItem) {
        ...
        let saveAction = UIAlertAction(title: "Add", style: .default) { (_) in
            self.addBook(name: popup.textFields![0].text!, topic: popup.textFields![1].text!, author: popup.textFields![2].text!, isRead: false, timestamp: Date(), image: self.randomImage)
            self.fetchBooks()
        }
